...
function SixVerseCard({ name, index }: { name: keyof typeof sixPalaceKnowledge; index: number }) {
  ...
  return <details className="six-verse-card" open={index === 0}>
    <summary><span className="six-verse-title">{name}</span><span>{knowledge.polarity}</span><span>{knowledge.element}</span><span className="six-verse-keywords data-note">{keywords}</span><span className="collapse-icon" aria-hidden="true">⌄</span></summary>
    <div className="six-verse-content"><div className="six-verse-text"><div><h4>口诀原文</h4><p>{knowledge.verse}</p></div><div><h4>现代解释</h4><p>{knowledge.verseModernMeaning}</p></div></div><div><h4>事项细断</h4><div className="topic-meaning-grid">{topics.map(([topic, meaning]) => <article key={topic}><strong className="pass-label">{specificTopicLabels[topic as keyof typeof specificTopicLabels] ?? '其他事项'}</strong><p>{meaning.initial}</p><p>{meaning.process}</p><p>{meaning.outcome}</p></article>)}</div></div></div>
  </details>
}
...
export function RulesPage() {
  ...
  return <section className="content-panel rules-page" aria-labelledby="rules-title">
    ...
    <section className="rules-section"><h3>六宫口诀与事项细断</h3><p>口诀原文与现代解释并列展示。方位、时段和结果只作传统类象参考，不参与三传落宫计算；健康内容不能用于诊断、疗效或生死判断。</p><p className="error-text rules-warning">健康类内容不能用于疾病诊断、疗效判断或生死预测，请以医生和实际检查结果为准。</p><p>口诀存在版本和流派差异；危险古断语仅作为原文展示，不作为现实事实。</p><div className="six-verse-grid">{Object.keys(sixPalaceKnowledge).map((name, index) => <SixVerseCard key={name} name={name as keyof typeof sixPalaceKnowledge} index={index} />)}</div></section>
    <section className="rules-section"><h3>日时双宫</h3><p>仅用于六宫时间起课：日宫在前、时宫在后，顺序不同含义不同；不改变三传落宫，也不用于数字、文字、随机或九宫起课。资料存在流派差异，原文仅作折叠查看。</p><p className="error-text rules-warning">日时双宫仅用于六宫时间起课。传统口诀存在流派差异，方位、时效和结果只作传统文化参考，不作确定判断。</p><div className="day-hour-groups">{Array.from(new Set(traditionalPairs.map((pair) => pair.dayPalace))).map((day) => <DayHourGroup key={day} day={day} pairs={traditionalPairs.filter((pair) => pair.dayPalace === day)} />)}<details className="day-hour-group"><summary><span>同宫组合 · 6组现代推导</span><span className="collapse-icon" aria-hidden="true">⌄</span></summary><div className="day-hour-pairs">{derivedSamePairs.map((pair) => <article className="day-hour-pair" key={`${pair.dayPalace}-${pair.hourPalace}`}><header><strong>{pair.dayPalace}＋{pair.hourPalace}</strong><span className="pair-type data-note">同宫推导</span></header><p>现代解释：{pair.modernMeaning}</p></article>)}</div></details></div></section>
