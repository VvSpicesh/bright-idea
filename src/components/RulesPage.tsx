...
export function RulesPage() {
  ...
  return <section className="content-panel rules-page" aria-labelledby="rules-title">
    ...
    <section className="rules-section"><h3>宫位顺序与解说配置</h3><p>以下内容直接读取当前规则和解说配置；没有配置的字段显示“未设定”。</p><div className="palace-rule-grid">{system.palaces.map((palace) => {
      ...
      return <article className="palace-rule-card" key={palace.index}><header><span>{palace.index + 1}</span><h4>{palace.name}</h4><b>{palace.element}</b></header><dl>{sixKnowledge ? <><div><dt>阴阳／五行</dt><dd>{sixKnowledge.polarity}／{sixKnowledge.element}</dd></div><div><dt>方位／神兽</dt><dd>{sixKnowledge.direction}／{sixKnowledge.spirit}</dd></div><div><dt>宫位</dt><dd>{sixKnowledge.palace}</dd></div><div><dt>关键词</dt><dd>{palace.keywords.join('、')}</dd></div><div><dt>通用含义</dt><dd>{sixKnowledge.generalMeaning}</dd></div><div><dt>事项领域</dt><dd>{Object.entries(sixKnowledge.topicMeanings).map(([topic, meaning]) => `${topic}：${meaning}`).join('；')}</dd></div><div><dt>人物特征</dt><dd>{sixKnowledge.personalityMeaning}</dd></div><div><dt>人体对应</dt><dd>{sixKnowledge.bodyMeaning}</dd></div><div><dt>天干／地支</dt><dd>{sixKnowledge.heavenlyStems}／{sixKnowledge.earthlyBranches}</dd></div><div><dt>象数对应</dt><dd>{sixKnowledge.numberSymbols.join('、')}（仅展示，不参与起课计算）</dd></div><div><dt>行动建议</dt><dd>{sixKnowledge.advice}</dd></div></> : <><div><dt>方位</dt><dd>{palace.direction || '未设定'}</dd></div><div><dt>关键词</dt><dd>{palace.keywords.join('、') || '未设定'}</dd></div><div><dt>初传含义</dt><dd>{semantics?.first || '未设定'}</dd></div><div><dt>中传含义</dt><dd>{semantics?.middle || '未设定'}</dd></div><div><dt>末传含义</dt><dd>{semantics?.last || '未设定'}</dd></div><div><dt>行动建议</dt><dd>{semantics?.advice || '未设定'}</dd></div></>}</dl></article>
    })}</div></section>
