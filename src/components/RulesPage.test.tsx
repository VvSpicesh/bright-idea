...
describe('规则页面', () => {
  it('reads both current rule systems and interpretation configuration', () => {
    ...
    expect(document.querySelectorAll('.six-verse-card')).toHaveLength(0)
    expect(document.querySelectorAll('.day-hour-group')).toHaveLength(0)
    expect(screen.queryByText('六宫口诀与事项细断')).not.toBeInTheDocument()
    expect(document.getElementById('nine-overview')).toBeInTheDocument()
  })
})
