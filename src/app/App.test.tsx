...
describe('三数起课应用流程', () => {
  ...
  it('shows a six-palace result and keeps inputs when returning', () => {
    ...
    expect(screen.queryByRole('heading', { name: '综合结论' })).not.toBeInTheDocument()
    ...
    expect(screen.getByRole('heading', { name: '三传解读' })).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '五行关系' })).toBeInTheDocument()
