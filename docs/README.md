...
- [产品规格 v0.1](product-spec.md)：早期完整产品目标；其中历史、复盘、设置、保存等内容尚未实现。
...
- [首版验收与单体测试](acceptance.md)：早期验收目标；历史记录相关验收尚未实现。
...
| 历史记录、复盘、JSON 导入导出 | “记录”页显示“记录功能后续开放” | 尚未实现 |
| 规则说明页和设置页 | “规则”页为简短占位内容，无设置页 | 尚未实现完整页面 |
| 结果保存、复制和完整计算轨迹文本 | 当前有重新起课、动画重播/跳过和 AI 提示词复制；没有保存结果、复制结果或独立轨迹列表 | 部分实现 |
| 每传结果卡展示宫位关键词 | 当前结果卡显示“观察基础条件/推进变化/收尾条件”，关键词集中在“判断依据” | 旧规格已过时 |
| 无网络可查看历史 | 没有历史记录功能 | 无法验收 |
